...
import type { ExportToolOutput } from "./types.js";
import {
  extractNoteTitle,
  NotesApiError,
  NotesClient,
    ...
    defineTool({
      ...
      parameters: {
        ...
        folder: {
          type: "string",
          description:
            "（写入模式）文件夹 ID 或精确名称；none/null/未分类 表示清除分类。省略时新建便签归为未分类。",
        },
        starred: {
          type: "boolean",
      ...
      timeoutMs: 60_000,
      execute: async (args, exec): Promise<ExportToolOutput> => {
        try {
