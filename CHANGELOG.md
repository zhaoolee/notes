# 更新日志

本文件记录锤子便签中值得用户关注的变更，格式参考 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/)，版本号遵循[语义化版本](https://semver.org/lang/zh-CN/)。

## [1.9.1] - 2026-08-24

### 修复

- “老罗巴扎嘿”的正文与标题统一改用项目内置的 OPPO Sans，避免 PNG 导出容器回退到文泉驿正黑与 Liberation Sans；网页预览、保存图片、离线归档和微信公众号富文本同步使用同一字体声明。

## [1.9.0] - 2026-08-24

### 新增

- 新增“老罗巴扎嘿”便签主题，入口位于 Bear 便签下方；它把 `editing-skill` 的暖米色杂志排版适配到通用 Markdown，并同步支持网页预览、保存图片、离线归档和微信公众号富文本。

## [1.8.4] - 2026-08-22

### 修复

- 修复公众号草稿中的引用被微信附加默认灰色左边线、与主题大引号叠成双重装饰的问题；除原本使用黑色引用线的 Telegra.ph 外，其他主题会明确清除微信默认引用边框和缩进。

## [1.8.3] - 2026-08-22

### 修复

- 修复无正文图片的文章发布公众号草稿时，生产后端读取不到固定默认封面并提示 `ENOENT`、无法保存草稿的问题；无图文章现在会提取便签标题的前 20 个可见字符，按当前主题即时生成专属封面，不再依赖运行镜像中的固定 Logo 文件。

## [1.8.2] - 2026-08-22

### 修复

- 进一步收紧公众号长文排版：iPhone、Bear 与 Telegra.ph 主题不再携带仅供锤子主题使用的四角表格，空行、引用、标题、工具栏和署名继续移除可继承声明；指定的十节文章 HTML 从 36600 个字符缩短到 11913 个，保留完整主题的同时留出约 40% 的微信长度余量。

## [1.8.1] - 2026-08-22

### 修复

- 修复正文并不长、但公众号主题反复写入相同段落样式后被误判为“超过 2 万字符”的问题；发布草稿现在让段落安全继承文章容器的字体、字号、颜色和行高，保持当前主题排版的同时显著缩短 HTML。

## [1.8.0] - 2026-08-22

### 新增

- 每个登录账号现在都可以在“设置 → 工具与扩展”中配置自己的微信公众号 AppID 与 AppSecret；两项会按账号隔离保存到服务端，并在重新打开设置时完整回显，AppSecret 默认以密码形式遮挡且可手动显示。公众号接口配置不再读取 `.env` 中的 AppID 或 AppSecret。
- 当前账号的公众号配置通过微信接口连通后，分享面板会在“复制到公众号”下方显示“发布为公众号草稿”；点击后会复用当前文章与所选主题的公众号富文本，将正文图片自动转成微信要求的小于 1MB 的 JPG/PNG 并换成微信专用地址、上传文章首图或默认图作为永久封面，再把结果保存到该账号的微信公众号草稿箱。

### 安全

- 公众号配置接口不接受前端指定账号 ID，只按当前登录会话读写，账号之间不能互相取得 AppSecret；匿名访问会被拒绝，响应禁止缓存。
- 公众号连通检查和草稿发布只能使用当前签名会话对应账号的配置，AppSecret 与接口调用凭据不会发送给浏览器中的分享功能；草稿发布拒绝跨站请求。

## [1.7.2] - 2026-08-18

### 修复

- 修复桌面分享预览顶部操作按钮在空间变窄时文字逐字换行的问题；取消、复制、归档和保存图片按钮现在始终保持单行显示。

## [1.7.1] - 2026-08-17

### 修复

- 修复在编辑器粘贴普通网址时被误当作远程图片导入并提示“不支持该图片格式”的问题；普通网址现在保留为正文，只有图片文件、富文本图片或扩展名明确的受支持图片地址才进入图片导入流程。

## [1.7.0] - 2026-08-17

### 新增

- 新增 `dsh-plugin/`：DeepSeek Harness 工具插件 `@zhaoolee/dsh-notes`，可通过 `dsh plugin --profile web add @zhaoolee/dsh-notes` 安装。安装后智能体获得 `notes_export_conversation` 工具，可把对话内容整理为 Markdown 导出为锤子便签（新建或按 `note_id` 更新当前账号云端工作区中的便签），并返回便签标题与可打开的前端链接。插件为分级模式：配置 `NOTES_API_BASE_URL` + 凭据时写入用户自己的便签列表；未配置任何用户信息时，回退到默认演示服务器 `notes.fangyuanxiaozhan.com` 的匿名导出接口，把对话渲染成锤子便签长图 PNG 返回本地文件路径（开箱即用尝鲜，不能写入便签列表）。

### 安全

- DSH 插件导出的本地便签图片现在保存在随机私有临时目录中；支持 POSIX 权限的系统仅允许当前用户访问目录和读写图片，避免同一设备上的其他用户读取对话内容。

### 修复

- iPhone 备忘录深色与浅色主题的顶部装饰恢复接近 Apple 原生导航栏的视觉尺寸：“备忘录”返回区及分享、新建图标不再缩成半尺寸，网页预览、保存图片和离线归档保持一致。
- 修复 AI 辅助审阅把空格放在 Markdown 加粗结束标记内侧时，预览直接显示 `**` 星号而无法渲染粗体的问题；现在会在建议进入正文前自动校正标记边界，同时保留原有空白和代码内容。
- 修复 DSH 插件市场从 GitHub 源码安装 `notes` 时，因默认禁止本地构建且仓库未携带 `lib/` 产物而提示“nothing installable”的问题；插件从 `0.1.2` 起随源码提交预编译入口，无需开启 `allowBuilds` 即可通过市场安装。
- 修复 `@zhaoolee/dsh-notes@0.1.0` 在 DSH `0.1.0-rc.6` 中首次执行 `notes_export_conversation` 就因重复安装宿主工具运行时而中止会话的问题；插件从 `0.1.1` 起复用 DSH 自带运行时。

## [1.6.1] - 2026-08-05

### 修复

- 修复生产环境中“单图聚焦”和“多页层叠”宣传页因嵌套路由错误加载静态资源而显示空白的问题。

## [1.6.0] - 2026-08-05

### 新增

- 新增“单图聚焦”和“多页层叠”两个独立宣传截图制作页，可在浏览器本地上传图片、编辑标题和说明、实时预览，并导出固定 2048 × 920 像素的高清 PNG；单图模板始终完整显示图片宽度，支持从顶部到底部调整长图取景，多页模板默认以第一张图的宽高比统一生成三层纸张、完整保留每张图片，也可手动切换为铺满页面并继续统一微调纸张宽度与高度。

### 变更

- 页面滚动条改为按需显示：滚动、悬停滚动区域或使用键盘聚焦时出现，空闲后自动隐藏，减少持续的视觉干扰。

### 修复

- iPhone 备忘录深色与浅色主题的标题、段落和引用现在与顶部“备忘录”工具栏共用同一条左右对齐线；预览、保存图片和离线归档保持一致。
- Telegra.ph 主题的引用整体向右微调，引用线不再紧贴正文左边界；网页预览、保存图片、离线归档和公众号排版保持一致。
- Bear 主题的引用红线现在与正文共用同一条左侧对齐线，引用文字仍保留清晰缩进；网页预览、保存图片和离线归档保持一致。
- 修复多页纸张跟随第一张图片变为横向比例后，图片仍按旧竖向坐标裁切而被截成窄条的问题；完整显示和铺满页面现在都按变换后的真实纸张比例计算取景。
- 多页宣传图的默认说明文案移除“最多上传三张图片”等操作限制，成品只保留面向读者的展示文案；上传数量仍在编辑区单独提示。
- 多页宣传图选择“铺满页面”时改为默认从图片顶部向下取景，并为前、中、后三层图片分别提供独立的上下取景滑杆，可各自移动到长图中部或底部。
- 单图宣传模板恢复参考设计中的手写文字背景纹理，并让该纹理随实时预览一起绘制到导出的高清 PNG；素材尚未加载时仍保留简化装饰作为回退。
- 多页宣传模板新增图片顺序管理，可把任意图片前移或后移到前、中、后三层；调整顺序时该图片的上下取景位置会一起移动。
- 两个宣传模板的背景颜色支持直接输入 `#debd85` 这类六位十六进制颜色，也可省略井号；输入完整有效值后会实时更新预览和导出画布。
- 多页宣传模板的默认标题和说明文字更新为“可能是史上最漂亮的便签应用”及对应的图文混排介绍文案。

## [1.5.1] - 2026-08-05

### 修复

- Bear 主题的粗体改用与超链接相同的红色，在预览、保存图片、离线归档和公众号排版中都更容易辨认；另外五种主题保持原有粗体颜色。

## [1.5.0] - 2026-08-05

### 新增

- 新增第六种 `Telegra.ph` 卡片主题，按原站的衬线正文、无衬线标题、紧凑段距、黑色引用线与浅色代码块重新排版；预览、保存图片、离线归档和公众号复制均使用同一套独立样式。

### 变更

- 预览右下角入口及浮窗标题统一由“配色／卡片配色”更名为“主题”，相关辅助说明和服务版本提示同步使用主题措辞。
- Bear 主题在主题按钮和主题列表中改用红底白熊的官方风格图标，更容易与其他卡片主题区分。
- Telegra.ph 主题在主题按钮和主题列表中改用官网声明的黑色 T 字母图标，替换原来的三条横线占位图。
- iPhone 备忘录浅色主题在主题按钮和主题列表中改用新的黄白备忘录图标；深色主题复用同一图标并叠加暗色遮罩，保持明暗入口的轮廓一致。
- 锤子便签亮色主题在主题按钮和主题列表中改用新的纸张铅笔图标；暗色主题复用同一图标并叠加暗色遮罩。
- 主题列表中的 iPhone 备忘录主题改为浅色在上、深色在下，顺序与常见的明暗展示习惯一致。
- 浏览器标题现在会区分访问方式：域名访问显示“开源版锤子便签”，IPv4 或 IPv6 地址访问显示“本地化开源版锤子便签”。

### 修复

- Bear 便签按原生 HTML 的 `15px / 1.755` 正文重新校准，并单独收紧 Markdown 空行与分节间距；预览、保存图片、离线归档和公众号排版不再出现被重复放大的纵向空白，其他四种样式保持不变。
- 修复桌面端更新日志页面无法使用鼠标滚轮滚动的问题；现在可直接在正文区域连续滚动，不再需要拖拽浏览器右侧滚动条。
- 修复手机分享面板打开后“主题”浮动按钮仍显示在面板上方的问题；按钮现在会正确位于遮罩与贴底操作层下方。

## [1.4.0] - 2026-08-05

### 新增

- 编辑界面现在会在正文末行后始终保留两行完整纸面空间，让最后一个可输入行停在倒数第三行，并同时适配桌面与手机行高。
- 便签预览新增移动端友好的浮动配色入口，可在暖白质感、深夜便签、iPhone 备忘录深浅模式与 Bear 极简排版之间切换；配色只影响预览卡片，并同步用于保存图片。
- 项目根目录新增可直接复制到便签编辑器的 Markdown 全样式测试模板，覆盖常用 Markdown、GFM 扩展、项目自定义分节与兼容性边界。

### 修复

- 五种便签样式现在相互隔离，并会随当前选择同步用于保存图片、离线归档和复制到微信公众号；不再把后两种产物固定生成为暖白锤子便签。
- iPhone 备忘录深色与浅色卡片统一改为直角纸面，不再保留圆角。
- Bear 配色恢复原版引用线的内容盒高度与细边框，红线现在略高于文字；同时校准标题字重、标题上下留白和完整空行节奏。
- iPhone 备忘录右上角改用 Apple iCloud Notes 官方的分享与新建备忘录矢量轮廓，移除错误的圆圈三点图标，并修正预览缩放和图片导出时的比例与对齐。
- iPhone 备忘录浅色模式的内置锤子 Logo 改为与暖白质感相同的原始样式，不再额外添加灰度、亮度滤镜或降低不透明度。
- 保存图片现在会校验后端实际采用的卡片主题；前后端版本不一致时明确提示重启导出服务，不再把未知的 iPhone 配色静默导出成暖白便签。

## [1.3.3] - 2026-08-04

### 变更

- 便签底部默认署名改为“由开源版锤子便签发送 / Powered by zhaoolee/notes”，已有默认设置也会自动更新。

### 修复

- 复制到微信公众号的正文、引用和标题改用紧凑字号，并移除全局文字描边、降低粗体字重，避免发布后整篇文字过大过粗。

## [1.3.2] - 2026-08-03

### 变更

- 设置“工具与扩展”中的 Hermes Skill 改用专属人物图片图标，不再显示字母 H。
- Hermes Skill 的安装指令、链接说明和操作按钮现在铺满扩展卡片可用宽度，减少长指令换行并改善信息层级。
- Hermes Skill 安装指令移除容易误解为引用块的左侧竖线，并降低正文的字号与颜色层级，让主要标题和操作更突出。

### 修复

- 跨端同步文章排序时会继续选中当前设备正在查看的同一篇便签，不再停留在原列表位置并自动切换正文。

## [1.3.1] - 2026-08-03

### 修复

- AI“重点加粗”现在会被明确要求避开中文句末标点与后续正文之间的无效 Markdown 边界，降低预览把加粗标记显示成星号的概率。

## [1.3.0] - 2026-08-02

### 新增

- 设置中的“账号与同步”现可导出全部便签：ZIP 会保留文件夹层级，把每张便签保存为 Markdown 并打包相关图片；导出期间会显示实时进度。

### 修复

- 公众号图片上传改用七牛官方 Node SDK，自动发现 Bucket 区域并在多个上传节点间故障转移；单个节点短时断线时不再原地重试三次后直接放弃。
- 七牛上传彻底失败时保留 `ECONNRESET` 等底层网络原因，便于从生产日志区分 DNS、连接和响应阶段故障。

## [1.2.0] - 2026-08-02

### 新增

- 设置新增“关于”栏目，可直接访问项目的 GitHub 开源地址和更新日志。

## [1.1.0] - 2026-08-02

### 新增

- 新增“自动适应”外观选项，可跟随系统浅色或深色模式实时切换暖白纸感和深夜便签。
- 新增 `/changelog` 独立页面，以锤子便签预览样式展示项目更新记录。
- Hermes Skill 设置新增可直接发送给 Hermes 的完整安装指令，并支持复制后在多台设备安装。

### 变更

- “AI 辅助审阅”设置已移至“工具与扩展”的“智能工具”分组，让“常规”页面专注于外观等基础设置。
- AI 审阅第三个快捷项改为“通俗化润色”，可拆分复杂长句并用简单概念表达专业内容。
- “纠正标点语法”检查后没有修改建议时，会明确提示“大模型已检查，无需纠正”。
- “自动适应”外观图标改为无描边的明暗双三角拼合样式，并保留与其他主题图标一致的圆角。
- PC 版设置入口已移至左上角三横菜单，分类栏底部不再重复显示设置齿轮。
- PC 版便签操作栏右侧按钮调整为“AI、删除、分享”的顺序，与 APP 版详情操作保持一致。
- 重置 Hermes 安装链接前会明确提示旧链接立即失效，并要求二次确认，避免误触轮换。

[1.9.1]: https://github.com/zhaoolee/notes/compare/1.9.0...1.9.1
[1.9.0]: https://github.com/zhaoolee/notes/compare/1.8.4...1.9.0
[1.8.4]: https://github.com/zhaoolee/notes/compare/1.8.3...1.8.4
[1.8.3]: https://github.com/zhaoolee/notes/compare/1.8.2...1.8.3
[1.8.2]: https://github.com/zhaoolee/notes/compare/1.8.1...1.8.2
[1.8.1]: https://github.com/zhaoolee/notes/compare/1.8.0...1.8.1
[1.8.0]: https://github.com/zhaoolee/notes/compare/1.7.2...1.8.0
[1.7.2]: https://github.com/zhaoolee/notes/compare/1.7.1...1.7.2
[1.7.1]: https://github.com/zhaoolee/notes/compare/1.7.0...1.7.1
[1.7.0]: https://github.com/zhaoolee/notes/compare/1.6.1...1.7.0
[1.6.1]: https://github.com/zhaoolee/notes/compare/1.6.0...1.6.1
[1.6.0]: https://github.com/zhaoolee/notes/compare/1.5.1...1.6.0
[1.5.1]: https://github.com/zhaoolee/notes/compare/1.5.0...1.5.1
[1.5.0]: https://github.com/zhaoolee/notes/compare/1.4.0...1.5.0
[1.4.0]: https://github.com/zhaoolee/notes/compare/1.3.3...1.4.0
[1.3.3]: https://github.com/zhaoolee/notes/compare/1.3.2...1.3.3
[1.3.2]: https://github.com/zhaoolee/notes/compare/1.3.1...1.3.2
[1.3.1]: https://github.com/zhaoolee/notes/compare/1.3.0...1.3.1
[1.3.0]: https://github.com/zhaoolee/notes/compare/1.2.0...1.3.0
[1.2.0]: https://github.com/zhaoolee/notes/compare/1.1.0...1.2.0
[1.1.0]: https://github.com/zhaoolee/notes/compare/1.0.0...1.1.0
