...
- 普通用户：由管理员使用用户名或邮箱创建，登录后使用独立的服务端工作区并跨
  设备同步。
- 超级管理员：访问 `/superadmin`，使用服务端 `.env` 中的
  `SUPERADMIN`、`SUPERADMINPASSWORD` 登录，负责创建、查看和重置普通用户密码。
...
工作区以不可猜测的用户 UUID 为键；`GET/PUT /api/workspace` 只接受签名 Cookie
中的用户 ID，不接受请求体传入用户 ID，因此普通用户无法选择或覆盖其他用户的
工作区。
...
- 无普通用户会话：继续读写原来的浏览器本地工作区。
- 有普通用户会话且云端已有数据：云端工作区覆盖当前页面状态。
- 有普通用户会话但云端为空：把当前浏览器工作区作为该账号的初始云端工作区。

登录后的本地变更延迟 650ms 写入云端；没有待保存变更时，每 15 秒读取一次云端
版本，较新的 `updatedAt` 会替换当前页面状态。当前策略为服务端时间戳驱动的
最后写入者优先，不提供逐便签合并。
...
已经指向当前七牛域名的图片直接复用，不产生新上传和额度消耗。普通登录用户使用
长期对象前缀，不执行匿名额度检查。

## 反馈测试

- `backend/tests/auth-feedback.test.ts`：管理员登录、用户名与邮箱建号、邮箱
  大小写不敏感登录、普通用户改密、管理员重置、密码不出现在用户列表和持久化
  明文中、旧会话失效，以及两个普通用户的工作区隔离。
- `backend/tests/quota-feedback.test.ts`：500 张共享额度、超额提示和北京时间
  0 点重置。
- `backend/tests/api-feedback.test.ts`：匿名临时上传策略，以及普通登录用户的
  长期上传和无限额分支。
