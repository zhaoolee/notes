...
卡片 `#EFE9DE`、分隔线 `#E6DFD8`、强调线 `#D4734B`。正文与标题统一使用
OPPOSans，正文保持 `15px / 1.8` 的排版节奏，二级标题使用 `■` 杂志标记，三级标题使用珊瑚色
...
- `POST /api/wechat`：上传文章图片到七牛并按 `theme` 生成带内联样式的公众号富文本；
  ...
  实际采用的 `theme`
- `POST /api/wechat/draft`：登录账号把当前 Markdown、所选 `theme` 和底部署名交给
  共享排版组件生成公众号富文本，直接将原图上传到微信、复用首图数据准备永久
  封面并保存到该账号的微信公众号草稿箱，不再经七牛中转
...
上述可配置署名。除 Telegra.ph 明确保留黑色引用线外，公众号引用必须以内联样式
显式清除 `blockquote` 的边框、内边距和斜体，避免微信保存后注入默认灰色左边线。
首行 `[文字]` 会去除方括号，
...
点击发布后，`POST /api/wechat/draft` 使用 `prepareWechatDraftArticle`，与
“复制到公众号”共用 `renderWechatArticle` 的主题、署名和内联排版。草稿不再调用
七牛上传：原始图片先映射为仅在本次渲染中使用的短占位地址，避免 Data URL 被
Markdown 过滤或计入正文长度，随后直接读取原图并替换为微信图片地址。同源
`/images/*` 直接读本地存储，外部图片按原地址下载；自定义署名 Logo 同样处理。
占位地址不能发给微信。复制到公众号仍使用七牛公开直链与原有匿名额度规则。
由于微信草稿接口会过滤外部图片，服务端继续扫描渲染后的 HTML。普通图片
...
`media_id`，其他首图复用正文阶段保留的原图数据，单独上传为永久素材，不再次下载。
只在单次请求中保留首图数据，不建立跨账号或长期图片缓存。
首图不符合永久素材要求或文章无图时，
...
成功响应的 `Server-Timing` 包含准备、凭据、正文图片、封面、保存与总耗时；服务端
记录相同阶段的毫秒数、总耗时和状态码，失败时记录已完成阶段。计时日志不包含
文章内容、图片地址、账号标识或凭据。
...
公众号 HTML 继续使用内联样式以适配微信编辑器。正文段落会直接携带当前主题的行高，
避免微信编辑器重建段落后用紧凑默认值覆盖；字体、字号、颜色、字重、字间距和断行规则
仍从文章容器继承，不会在每一个段落与空行上重复写入整套声明。长度校验在这种无损收紧
之后执行，因此真实正文仍受 2 万字符限制，而十节
左右的常规长文不会仅因主题样式重复而被误判超限。锤子明暗主题用普通容器绘制四角
和双线边框；边框本来就是透明的 iPhone、Bear 与 Telegra.ph 主题直接渲染内容容器。
装饰布局不能使用表格；只有正文中的真实 Markdown 表格输出 `table`。
...
空心方格 `span`，并填充不可见的不换行空格。上下各一个普通 `section` 使用 flex
将两个角放在左右两端，以 `-1px` 纵向间距与外框连接，不依赖绝对定位。
外框 `section` 使用 `0 5px` 左右外边距和 `2.8px` 内边距，正文区绘制唯一内框；
四个方格位于主框之外：左上方格以右下角、右上方格以左下角、左下方格以右上角、
右下方格以左上角连接外框，保留 `example/程序员狠话Vol.5.JPG` 的成品几何。
纸面和角标行不画边框，只有内外两框和四个角方格带边框。
不要恢复装饰表格：微信会为表格补入空 `caption`，已观察到其默认顶部样式为
`1px dashed #ddd` 且占高 `7px`。普通容器避免纸框进入微信的表格加工流程；
微信后台首次保存、重新打开后连续保存的实际边框效果仍须作为线上验收步骤。
底部署名使用本地化的
...
标点语法”“重点加粗”“通俗化润色”三种可多选审阅要求；用户勾选的要求与可选补充
要求会合并为一个提示词，通过单次请求交给模型。其中通俗化模式会把复杂长句拆成
