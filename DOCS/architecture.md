...
公众号 HTML 以桌面成品便签的标准 `2×` 比例为唯一尺寸基准：纸张最大宽度为
`660px`，正文为 `24.32px / 1.8`，H1 为 `40.96px / 1.28`，H2 为
`29.44px / 1.35`；框体留白、段落空行、图片衬边和署名也按同一倍率内联展开，
不能再维护一套 `15px` 正文、标题下划线和引用色块的公众号紧凑样式。底部保留
...
标点语法”“重点加粗”“让公众更易读”三种审阅要求，其中公众易读模式只拆分长句，
不扩写且保留原意和 Markdown 结构。收到建议时不调用 `setMarkdown`，每张建议卡
